...
#ifdef OBSOLETE
/* Frees a TOKEN_GROUPS struct allocated by ObjToTOKEN_GROUPS */
void TwapiFreeTOKEN_GROUPS(TOKEN_GROUPS *tgP)
{
    DWORD i;
    if (tgP) {
...
/*
 * Returns allocated TOKEN_GROUPS structure which must be freed
 * by calling TwapiFreeTOKEN_GROUPS
 */
static int ObjToPTOKEN_GROUPS(Tcl_Interp *interp, Tcl_Obj *obj, TOKEN_GROUPS **tgPP)
{
    Tcl_Obj **objv;
    int       objc;
    int       i;
     ...
     * the SID and integer attributes
     */
    for (i = 0; i < objc; ++i) {
        if (ObjToSID_AND_ATTRIBUTES(interp, objv[i], &tgP->Groups[i]) == TCL_OK) {
            /* Valid entry. We update GroupCOunt as we go along in
             * case we have to free the structure in the middle in case
             * of errors
             */
            tgP->GroupCount += 1;
        }
        else {
            /* Free anything allocated so far and vamoose. */
            TwapiFreeTOKEN_GROUPS(tgP);
            return TCL_ERROR;
        }
    }
...
#ifdef OBSOLETE
/* TOKEN_SOURCE */
static int ObjToTOKEN_SOURCE(Tcl_Interp *interp, Tcl_Obj *obj, TOKEN_SOURCE *tsP)
{
    int       objc;
    Tcl_Obj **objv;
    char     *p;
    ...
    if ((objc != 2) ||
        (ObjToLUID(interp, objv[1], &tsP->SourceIdentifier) != TCL_OK)) {
        return TCL_ERROR;
    }

    p = Tcl_GetStringFromObj(objv[0], &len );
    if (len != sizeof(tsP->SourceName)) {
        if (interp)
