...
static int Twapi_CallOneArgObjCmd(ClientData clientdata, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
    ...
    case 1006: // reveal
        mark = SWSPushMark();
        s = ObjDecryptWinCharsSWS(interp, objv[0], &dw);
        if (s == NULL) {
            res = TCL_ERROR;
            goto vamoose;
        }
        result.value.obj = ObjFromWinCharsN(s, dw);
        result.type = TRT_OBJ;
    ...
    case 1007: // conceal
        ...
        s = ObjToWinCharsN(objv[0], &dw);
        result.value.obj = ObjEncryptWinChars(interp, s, dw);
        result.type = TRT_OBJ;
        break;
